...
import { StoryWithTranslations, createStory, deleteStory, updateStory } from './_actions'
import {
  Table,
  TableBody,
  TableCell,
...
export function StoryList({ initialStories }: StoryListProps) {
  ...
  const [selectedStory, setSelectedStory] = useState<StoryWithTranslations | null>(null)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-100">Stories</h2>
          <p className="text-slate-400 mt-1">Manage your story chronicles and translations.</p>
        </div>
          ...
          <DialogContent className="bg-slate-900 border-slate-800 text-slate-100">
            ...
            <form action={handleCreate} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="title" name="title" required placeholder="Story Title" className="bg-slate-950 border-slate-800 text-slate-100 placeholder:text-slate-500" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="description" className="text-slate-200">Description</Label>
                <Textarea id="description" name="description" required placeholder="Story Description" className="bg-slate-950 border-slate-800 text-slate-100 placeholder:text-slate-500" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="coverImageUrl" className="text-slate-200">Cover Image URL</Label>
                <Input id="coverImageUrl" name="coverImageUrl" placeholder="https://..." className="bg-slate-950 border-slate-800 text-slate-100 placeholder:text-slate-500" />
              </div>
              <div className="space-y-2">
          ...
          <Table>
            ...
            <TableBody>
              {initialStories.map((story) => {
                ...
                const authorName = story.author.translations[0]?.name || 'Unknown'

                return (
                  <TableRow key={story.id} className="border-slate-800 hover:bg-slate-800/50">
                    <TableCell className="font-medium text-slate-200">{translation?.title || 'Untitled'}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" className="h-8 w-8 p-0 text-slate-400 hover:text-slate-100 hover:bg-slate-800">
                            <span className="sr-only">Open menu</span>
                            <MoreHorizontal className="h-4 w-4" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="bg-slate-900 border-slate-800 text-slate-200">
                          <DropdownMenuItem onClick={() => openEdit(story)} className="focus:bg-slate-800 focus:text-slate-100 cursor-pointer">
                            <Pencil className="mr-2 h-4 w-4" /> Edit
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleDelete(story.id)} className="text-red-400 focus:bg-red-950/30 focus:text-red-300 cursor-pointer">
              ...
              {initialStories.length === 0 && (
                <TableRow className="border-slate-800">
                  <TableCell colSpan={5} className="text-center h-32 text-slate-500 hover:text-white transition-colors cursor-default">
        ...
        <DialogContent className="bg-slate-900 border-slate-800 text-slate-100">
          ...
          {selectedStory && (
            <form action={handleUpdate} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-coverImageUrl" className="text-slate-200">Cover Image URL</Label>
                <Input 
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-language" className="text-slate-200">Language</Label>
                <select
                  id="edit-language"
                  name="language"
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="edit-isPublished" className="text-slate-200">Published</Label>
              </div>
              <DialogFooter>
                <Button type="submit" disabled={isLoading} className="bg-violet-600 hover:bg-violet-700 text-white">
